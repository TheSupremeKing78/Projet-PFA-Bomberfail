...
let update _dt el =
  Seq.iteri
    (fun i (e1 : t) ->
      ...
      let pos1 = Rect.{width=box1.width;height=box1.height; x=pos1.x; y=pos1.y} in
      Seq.iteri
        (fun j (e2 : t) ->
          ...
          if j > i && (Float.is_finite m1 || Float.is_finite m2) then begin
            ...
            let pos2 = Rect.{width=box2.width;height=box2.height; x=pos2.x; y=pos2.y} in
            let s_pos, s_rect = Rect.mdiff pos2 box2 pos1 box1 in
            let s_pos = Rect.{width=box1.width; height=box1.height; x=s_pos.x; y=s_pos.y} in
            if
            ...
            then begin
              ...
              let pos1=Vector.{x=pos1.x;y=pos1.y} in
              let pos1 = Vector.add pos1 delta_pos1 in
              let pos2=Vector.{x=pos2.x;y=pos2.y} in
              let pos2 = Vector.add pos2 delta_pos2 in
              let pos1 = Rect.{width=box1.width;height=box1.height; x=pos1.x; y=pos1.y} in
              let pos2 = Rect.{width=box1.width;height=box1.height; x=pos2.x; y=pos2.y} in
              let s_pos, s_rect = Rect.mdiff pos2 box2 pos1 box1 in
              let s_pos = Rect.{width=box1.width; height=box1.height; x=s_pos.x; y=s_pos.y} in
              if Rect.has_origin s_pos s_rect then begin
                Gfx.debug "%f, %f, %d x %d\n" s_pos.Rect.x s_pos.Rect.y
